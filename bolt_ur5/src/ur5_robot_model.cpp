                               ...
                               const std::string& world_frame, const std::string& tcp_frame)
{
  ...
  if (!ur_kinematics::URKinematicsPlugin::initialize(robot_description, group_name, ikfast_base_link, ikfast_tip_link,
                                                     0.001))
  {
                               ...
                               const std::string& world_frame, const std::string& tcp_frame)
{
  ...
  if (!ur_kinematics::URKinematicsPlugin::initialize(robot_model, group_name, ikfast_base_link, ikfast_tip_link, 0.001))
  {
    ROS_ERROR_STREAM("URKinematicsPlugin from within UR5 robot model failed to initialize");
    return false;
  }
...
bool UR5RobotModel::initializeHelper(const std::string& group_name, const std::string& world_frame,
                                     const std::string& tcp_frame)
{
  ...
  {
    ROS_DEBUG_STREAM_NAMED(name_, "tcp_frame (" << tcp_frame << ") does not equal tip frame (" << getTipFrame()
                                                << "), creating transform");
  ...
  {
    ROS_DEBUG_STREAM_NAMED(name_, "world_frame (" << world_frame << ") does not equal base frame (" << getBaseFrame()
                                                  << "), creating transform");
