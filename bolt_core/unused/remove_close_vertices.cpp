...
bool SparseCriteria::checkRemoveCloseVertices(SparseVertex v1, std::size_t indent)
{
  ...
  BOLT_FUNC(vRemoveClose_, "checkRemoveCloseVertices() v1 = " << v1);
  ...
  if (vRemoveClose_)
  {
    ...
    BOLT_DEBUG(vRemoveClose_, "Neighbors of " << v1 << " are: [" << ss.str() << "]");
  }

  // Error check no neighbors
  if (graphNeighbors.size() <= 1)
  {
    BOLT_ERROR(vRemoveClose_, "checkRemoveCloseVertices: no neighbors found for sparse vertex " << v1);
    return false;
  }
  ...
  if (v1 == v2)
  {
    BOLT_ERROR(vRemoveClose_, "checkRemoveCloseVertices: error occured, cannot remove self: " << v1);
    exit(-1);
  }
  ...
  {
    // BOLT_DEBUG(vRemoveClose_, "Distance " << sg_->distanceFunction(v1, v2) << " is greater than max "
  ...
  {
    BOLT_ERROR(vRemoveClose_, "checkRemoveCloseVertices: not collision free v1=" << v1 << ", v2=" << v2);
    return false;
  }

  BOLT_DEBUG(vRemoveClose_, "Found visible nearby node, testing if able to replace " << v2 << " with " << v1);
  ...
  foreach (SparseEdge edge, boost::out_edges(v2, sg_->getGraph()))
  {
    ...
    BOLT_DEBUG(vRemoveClose_, "checking edge v1= " << v1 << " to v3=" << v3);

    // Check if distance is within sparseDelta
    if (si_->distance(sg_->getState(v1), sg_->getState(v3)) > sparseDelta_)
    {
      BOLT_DEBUG(vRemoveClose_, "checkRemoveCloseVertices: distance too far from previous neighbor " << v3);
      return false;
    }
    ...
    {
      BOLT_ERROR(vRemoveClose_,
                 "checkRemoveCloseVertices: not collision free path from new vertex to potential neighbor " << v3);
      return false;
    }
  }

  BOLT_DEBUG(vRemoveClose_, "Found qualified node to replace with nearby");
  ...
  {
    SparseVertex v3 = boost::target(edge, sg_->getGraph());
    BOLT_DEBUG(vRemoveClose_, "Connecting v1= " << v1 << " to v3=" << v3);
    sg_->addEdge(v1, v3, eINTERFACE, indent + 4);
  }

  // Delete old vertex
  sg_->removeVertex(v2, indent);
  BOLT_DEBUG(vRemoveClose_, "REMOVING VERTEX " << v2 << " which was replaced with " << v1 << " with state ");
