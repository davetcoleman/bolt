...
//=======================================================================
//
#ifndef OMPL_BOOST_DISJOINT_SETS_HPP
#define OMPL_BOOST_DISJOINT_SETS_HPP
...
public:
  ...
  // Remove Set -- Delete the singleton set containing vertex x
  template <class Element>
  inline void remove_set(Element x)
  {
    // Check if this element is the representative of its set
    // if (get(parent, x) == x)
    // {
    //   std::cout << "Element " << x << " is rep of its set" << std::endl;
    // }
  ...
  {
    detail::my_link_sets(parent, rank, x, y, rep);
  }
  ...
  {
    for (; first != last; ++first)
      detail::my_normalize_node(parent, *first);
  }
...
public:
  ...
  {
    for (; first != last; ++first)
      detail::my_normalize_node(&parent[0], *first);
  }
